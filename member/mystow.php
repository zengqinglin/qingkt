...
if($rank == 'top'){
   ...
   $tpl = 'stowtop';
	...
	$sql = "SELECT s.*,t.* FROM `#@__member_stow` AS s left join `#@__archives` AS t on t.id=s.aid  where s.mid='".$cfg_ml->M_ID."' order by s.id desc";    
	...
	//$sql= "SELECT id,topid FROM `#@__arctype` WHERE typename LIKE '{$arr['typename']}' AND id<>'{$tid}' AND topid<>'{$tid}'  "
    $tpl = 'mystow';
}
