...
<div class='decmt-box2'>
  <ul>
    <li>
        ...
        <a href="<?php echo $spaceurl; ?>" target="_blank"><img src="<?php echo $mface;?>" width="40" height="40"></a>
        </div>
        <div class="notelist_content">
          <div class="u_name">
            <a href="<?php echo $spaceurl; ?>" target="_blank"><?php echo $username; ?></a>
          </div>
           <div style="clear:both">
          <p><?php echo ubb($msg); ?></p>
          </div>
          <div class="notelist-bottom clearfix">
            <span title="创建时间" class="l timeago">时间：<?php echo GetDateMk($dtime); ?></span>
                        
                        <div class="notelist-actions">
                   <span id='goodfb<?php echo $id; ?>' class="js-pl-praise list-praise "> <a title="赞" href='#goodfb<?php echo $id; ?>' onclick="postBadGood('goodfb',<?php echo $id; ?>);"> <span class="icon icon_good "></span></a>[<?php echo $good; ?>] </span>
...
<div id='commetpages' class='page'>";
  ...
  {
  echo ($j==$curpage ? "<a class='active'>$j</a> " : "<a href='#commettop' onclick='LoadCommets($j);'>{$j}</a> ");
  }
  if($curpage+1 <= $totalpage )
  {
  echo "<a href='#commettop' onclick='LoadCommets(".($curpage+1).");'>下一页</a> ";
  }
   echo "<span> {$allpage} 页/{$totalcount} 条评论</span> ";
  echo "</div>
";
}