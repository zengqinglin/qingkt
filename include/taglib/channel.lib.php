...
//die("gameover");
	...
	if($le&&$le==2){
		...
		//if($reid==0) return '';
        $sql = "SELECT id,typename,typedir,isdefault,ispart,defaultname,namerule2,moresite,siteurl,sitepath
            FROM `#@__arctype` WHERE reid=$myid And ishidden<>1 And topid<>reid order by sortrank asc limit 0, $line ";
    ...
    for($i=0;$i < $line;$i++)
    {
        ...
        for($j=0; $j<$col; $j++)
        {
            ...
            if($row=$dsql->GetArray())
            {
                ...
                if(($row['id']==$pageid&&$currentstyle!='') || ($row['id']==$typeid || ($topid==$row['id'] && $type=='top') ) && $currentstyle!=''||($le==1&&$refObj->Fields[reid]==$row['id']))
                {	
