...
class ListView
{
     ...
     * @return    string
     */
    function __construct($typeid,$iseasy,$uppage=1)
    {
        global $dsql,$ftp;
        $this->TypeID = $typeid;
        $this->dsql = &$dsql;
    ...
    function CountRecord()
    {
		...
		if($this->iseasy!=""){
			 $this->addSql  = "arc.arcrank > -1 AND (qkt_addonarticle.level =  '".$this->iseasy."')";
			}else{
        ...
        if($cfg_list_son=='N')
        {
            ...
            {
                if($this->CrossID=='') $this->addSql .= " AND (arc.typeid='".$this->TypeID."') AND (AND qkt_addonarticle.level =  '".$this->iseasy."')";
        ...
        {
			if($this->iseasy!=""){
			$cquery = "SELECT COUNT(*) AS dd FROM `#@__arctiny` arc  LEFT JOIN `qkt_addonarticle` ON arc.id 

= qkt_addonarticle.aid WHERE ".$this->addSql;
				}else{
    ...
    $imgwidth=120,$imgheight=90,$listtype="all",$orderby="default",$innertext="",$tablewidth="100",$ismake=1,$orderWay='desc')
    {
        ...
        else
        {
            ...
            $ids = array();
		if($this->iseasy!=""){
			 $query = "SELECT id FROM `#@__arctiny` arc  LEFT JOIN `qkt_addonarticle` ON arc.id 

= qkt_addonarticle.aid  WHERE {$this->addSql} $ordersql LIMIT $limitstart,$row ";
			}else{
            ...
            else
            {
				...
				if($this->iseasy!=""){
					$query = "SELECT arc.*,tp.typedir,tp.typename,tp.corank,tp.isdefault,tp.defaultname,
                       ...
                       $addJoin
                       WHERE arc.id  in($idstr) AND qkt_addonarticle.level='{$this->iseasy}' $ordersql ";
					$query2= "WHERE arc.id in($idstr) AND qkt_addonarticle.level='{$this->iseasy}' $ordersql ";
				...
				$query2="WHERE arc.id in($idstr)$ordersql ";
					}
            }
    ...
    function GetPageListST($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($totalpage<=1 && $this->TotalResult>0)
        {
            ...
            return "<span class=\"disabled_page\">共 <strong>1</strong>页<strong>".$this->TotalResult."</strong>条记录</span>\r\n";
        }
        if($this->TotalResult == 0)
        {
            return "<span class=\"disabled_page\">共 <strong>0</strong>页<strong>".$this->TotalResult."</strong>条记录</span>\r\n";
        }
        $purl = $this->GetCurUrl();
        $maininfo = "<span  class=\"disabled_page\">共 <strong>{$totalpage}</strong>页<strong>".$this->TotalResult."</strong>条</span>\r\n";
        ...
        {
            $indexpage="<span class=\"disabled_page\">首页</span>\r\n";
        }
        ...
        {
            $endpage="<span class=\"disabled_page\">末页</span>\r\n";
        }
    ...
    function GetPageListDM($list_len,$listitem="index,end,pre,next,pageno")
    {
        ...
        if($totalpage<=1 && $this->TotalResult>0)
        {
            return "<span class=\"disabled_page\">共 1 页/".$this->TotalResult." 条记录</span>\r\n";
        }
        if($this->TotalResult == 0)
        {
            return "<span class=\"disabled_page\">共 0 页/".$this->TotalResult." 条记录</span>\r\n";
        }
        $maininfo = "<span class=\"disabled_page\">共 <strong>{$totalpage}</strong>页<strong>".$this->TotalResult."</strong>条</span>\r\n";
        ...
        //$hidenform .= "<input type='hidden' name='TotalResult' value='".$this->TotalResult."'>\r\n";
		   $optionlist=" <b class=\"pager-cur\">$this->PageNo</b> / ".$totalpage."\r\n";
		
        //获得数字链接
        //获得上一页和下一页的链接
        if($this->PageNo != 1)
        {
            $prepage.="<a href='".$purl."PageNo=$prepagenum&&iseasy=$this->iseasy'>上一页</a>\r\n";
            $indexpage="<a href='".$purl."PageNo=1&&iseasy=$this->iseasy'>首页</a>\r\n";
        }
        else
        {
            $indexpage="<span class=\"disabled_page\">首页</span>\r\n";
        ...
        if($this->PageNo!=$totalpage && $totalpage>1)
        {
            $nextpage.="<a href='".$purl."PageNo=$nextpagenum&&iseasy=$this->iseasy'>下一页</a>\r\n";
            $endpage="<a href='".$purl."PageNo=$totalpage&&iseasy=$this->iseasy'>末页</a>\r\n";
        }
        else
        {
            $endpage="<span class=\"disabled_page\">末页</span>\r\n";
        ...
        for($j;$j<=$total_list;$j++)
        {
            ...
            {
                $listdd.="<a href='".$purl."PageNo=$j&&iseasy=$this->iseasy'>".$j."</a>\r\n";
				// var_dump( $listdd);
				//die("-------");
            }
        }
